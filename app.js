...
function layout(content){
  ...
  <main class="main">
    <header class="topbar"><div class="top-meta"><div class="date">${uiIcon('calendar')}<span>${esc(formatDate(r.date))}</span></div><div class="data-window">${esc(t('window.dataWindow',{start:r.window.start,end:r.window.end}))}</div><div class="data-updated">${esc(r.generated_at ? t('window.updated',{time:formatDateTime(r.generated_at)}) : t('window.updatedUnknown'))}</div>${fallback}</div><div class="top-actions"><label class="locale-control"><span class="sr-only">${esc(t('language.label'))}</span>${uiIcon('language')}<select id="localeSelect" aria-label="${esc(t('language.label'))}">${localeOptions()}</select></label><button class="icon-btn" id="themeToggle" aria-label="Theme">${uiIcon(state.theme==='night'?'sun':'moon')}</button><button class="pill-btn" data-route="#/my-radar">${uiIcon('user')}<span>${esc(t('nav.myRadar'))}</span></button></div></header>
