...
function today(){
  const r=state.report, top=topSignals(r), personal=personalSignals().slice(0,5);
...
function filteredSignals(){return state.report.signals.filter(s=>{
  ...
  return (!q || JSON.stringify(s).toLowerCase().includes(q)) && (state.filters.category==='all'||s.categories.includes(state.filters.category)) && (state.filters.region==='all'||effectiveRegions(s).includes(state.filters.region)) && (state.filters.severity==='all'||scoreClass(s.score)===state.filters.severity)
...
function myRadar(){const topicOptions=unique(state.report.signals.flatMap(x=>x.categories));return layout(`<div class="page-title"><div><h1>${esc(t('myRadar.title'))}</h1><p>${esc(t('myRadar.description'))}</p></div></div><div class="profile-grid"><div class="setting-group"><h3>${esc(t('myRadar.topics'))}</h3><div class="option-grid">${topicOptions.map(x=>`<button class="toggle-chip ${state.prefs.topics.includes(x)?'selected':''}" data-pref-topic="${x}">${esc(categoryLabel(x))}</button>`).join('')}</div></div><div class="setting-group"><h3>${esc(t('myRadar.regions'))}</h3><div class="option-grid">${REGION_CATALOG.map(x=>`<button class="toggle-chip ${state.prefs.regions.includes(x)?'selected':''}" data-pref-region="${x}">${esc(regionLabel(x))}</button>`).join('')}</div></div><div class="setting-group"><h3>${esc(t('myRadar.minScore'))}</h3><div class="option-grid">${[70,75,80,85,90].map(x=>`<button class="toggle-chip ${state.prefs.minScore===x?'selected':''}" data-pref-score="${x}">≥ ${x}</button>`).join('')}</div></div><div class="setting-group"><h3>${esc(t('myRadar.summary'))}</h3>${personalSignals().map(x=>`<div style="padding:8px 0;border-top:1px solid var(--line);font-size:12px" data-signal="${x.id}">• ${esc(x.title)}<div class="tags" style="margin-top:5px">${effectiveRegions(x).filter(r=>state.prefs.regions.includes(r)).slice(0,3).map(r=>`<span class="tag">${esc(regionLabel(r))}</span>`).join('')}</div></div>`).join('')||`<div class="empty">${esc(t('myRadar.empty'))}</div>`}</div></div>`)}
